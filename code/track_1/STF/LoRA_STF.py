...
运行示例：
python code/track_1/STF/LoRA_STF.py \
  --gpu 2,3,4,5 \
  --max_steps 5 \
  --save_steps 5 \
  --eval_steps 5
...
"""检查视频能否被当前 torchvision 解码，输入：视频路径、缓存字典 -> 输出：bool"""
def can_decode_video(video_path: str, cache: dict) -> bool:
    if video_path in cache:
        return cache[video_path]
    try:
        from torchvision.io import read_video
        frames, _, _ = read_video(video_path, start_pts=0, end_pts=1, pts_unit="sec")
        cache[video_path] = len(frames) >= 2
...
def check_required():
    ...
    for path in [model_path, train_path, val_path]:
        if not path.exists():
...
run_cmd(infer_cmd, infer_env, run_log_path)  # 输入：推理命令、单卡环境变量、运行日志；输出：验证集预测文件。

stats = compute_accuracy(filtered_val_path, prediction_path)  # 输入：验证集和预测文件；输出：准确率统计。
overall_acc = format_acc(stats["overall"])
final_stem = f"{MODEL_NAME}__{time_name}__ACC-{overall_acc}"
...
lines = [
    ...
    f"prediction_path: {final_prediction_path}",
    "",
    "LoRA parameters:",
    f"tuner_type: lora",
...
print(f"overall_accuracy: {overall_acc}", flush=True)
